...
type User struct {
	...
	Role             UserRole  `gorm:"type:enum('student', 'admin')" json:"role"`
	Avatar           string    `gorm:"type:varchar(255)" json:"avatar"`
	Points           int       `gorm:"default:0" json:"points"`
	RankTier         RankTier  `gorm:"type:enum('Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond')" json:"rank_tier"`
	MMR              int       `json:"mmr"`
...
type Friendship struct {
	...
	Status      FriendshipStatus `gorm:"type:enum('pending', 'accepted', 'blocked')" json:"status"`
	CreatedAt   time.Time        `json:"created_at"`
